...
class Bubble {
    ...
    getCanvasSize() { //Метод getCanvasSize будет как раз вытаскивать из холста его размеры и сохранять в переменные внутри класса Bubble.
        this.canvasWidth = this.canvas.clientWidth;
        this.canvasHeight = this.canvas.clientHeight;
    }

    init() { //Метод init будет инициализировать пузырек: выбирать ему один из случайных цветов, какой-то размер, начальное положение на холсте.
    ...
    move() { //метод move будет пересчитывать положение пузырька на холсте, так как фигуры должны двигаться вверх
